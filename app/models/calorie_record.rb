class CalorieRecord < ApplicationRecord
  ...
  before_validation :set_rice_kcal_per_bowl, on: :create
...
#（プロフィール変更によって過去の記録が変わらないようにするため
  def set_rice_kcal_per_bowl
    ...
    if user.profile.present?
      self.rice_kcal_per_bowl ||= user.profile.rice_kcal_per_bowl
    else
      self.rice_kcal_per_bowl ||= 0
    end
  end
  ...
  def calculate_total_calories
    ...
    self.calorie = calorie.to_i + rice_calorie
  end
end
